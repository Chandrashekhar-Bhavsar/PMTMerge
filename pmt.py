...
def store_otp(email, otp):
    # Use the global variables to store the OTP and email
    global stored_otp, reset_email
    stored_otp = otp
    reset_email = email
# Retrieve Stored OTP

def get_stored_otp(email):
    # Use the global variable to retrieve the stored OTP
    return stored_otp
# Send OTP to User

def send_otp_email(email, otp):
    # Configure email settings
    sender_email = "techbrutal11@gmail.com"
    smtp_server = "smtp.gmail.com"
    smtp_port = 587
    smtp_username = "techbrutal11@gmail.com"
    smtp_password = "pfpnaxhguopcukvc"
...
def get_cardprojectdetails():
    try:
        ...
        query = "SELECT p.Project_ID,p.Project_Name,p.Project_Description,p.Planned_SD,p.Planned_ED,p.Actual_SD,p.Actual_ED,p.Planned_Hours,p.Actual_Hours,p.Status,p.Project_Lead,p.Client_Name,p.Risk,p.Mitigation,p.ownby_id,p.ownby_name,s.status FROM Project_Details p JOIN project_member m ON m.Project_ID = p.Project_ID JOIN project_status s ON s.id = p.Project_ID WHERE p.ownby_id = m.user_ID AND m.user_ID = %s;"
        value=(user_ID,)
...
def add_attachments():
    try:
        now = datetime.now()
        dt_string = str(now.strftime("%d/%m/%Y %H:%M:%S"))
        logging.debug(dt_string + " Inside add_attachments api....")
        data = request.get_json()
        ...
        if "Name" not in data:
            return jsonify({"error": "Missing 'Name' in request data"}), 400
        logging.debug(dt_string + " Accepting values to add attachment ")
        issue_id=data["issue_id"]
        link = data["link"]
        Name = data["Name"]
        query = "Insert into attachment(issue_id,Name,link) values(%s,%s,%s);"
        values = (issue_id,Name,link)
        cursor.execute(query,values)
        return jsonify({"msg":"attachment added succesfully"}), 200
...
def display_attachments():
    try:
        now = datetime.now()
        dt_string = str(now.strftime("%d/%m/%Y %H:%M:%S"))
        logging.debug(dt_string + " Inside display_attachments api....")
        data = request.get_json()
        if "issue_id" not in data:
            return jsonify({"error": "Missing 'issue_id' in request data"}), 400
        logging.debug(dt_string + " Accepting values to display attachment ")
        issue_id=data["issue_id"]
        query = "select * from attachment where issue_id = %s;"
        values = (issue_id,)
        cursor.execute(query,values)
        id = cursor.fetchall()
        if not id :
            return ({"msg" : "no attachments associated"}),200
        comments_list = []
        for project in id:
                comments_dict = {
                    "sno" : project[0],
        ...
        logging.debug(dt_string + " Returning the list of all the attachments related to this issue...")
