...
# Print the results
        ...
        if type(Project_ID) is not int:
            return jsonify({"Error": "Wrong data type of project id"}), 400
        query1 = "SELECT count(i.issue_id) FROM project_issue i INNER JOIN Task t ON i.issue_id = t.issue_id WHERE i.Project_ID=%s and t.task_sd=%s ;"
        ...
        print("issue =",issue)
        logging.debug(dt_string+" Query Exectued successfully ")
        logging.debug(dt_string+" issue day API is executed successfully")
...
def issuereport():
    try:
        ...
        query1 = "select count(issue_id) from Issue_Details where status in ('COMPLETED' OR 'DONE') AND issue_id in (select issue_id from project_issue where Project_ID = %s) " 
        values1 = (Project_ID,)
        cursor.execute(query1, values1)
        completed=cursor.fetchall()
        
        query2 = "select count(issue_id) from Issue_Details where status NOT IN ('COMPLETED' OR 'DONE') AND issue_id in (select issue_id from project_issue where Project_ID = %s) " 
        values2 = (Project_ID,)
